...
export default function Clients() {
  ...
  const handleAdd = () => {
    ...
    addClient({ nom, telephone, adresse });
    toast.success(`Client "${nom}" ajouté.`);
    setShowAdd(false);
    setNom(""); setTelephone(""); setAdresse("");
  };

  const clientVentes = selectedClient
    ? ventes.filter((v) => v.clientId === selectedClient)
    : [];
        ...
        <Dialog open={showAdd} onOpenChange={setShowAdd}>
          ...
          <DialogContent className="bg-card border-border">
            ...
            <div className="space-y-4 mt-4">
              <div>
                <label className="label-industrial">Nom complet *</label>
                <input className="input-underline w-full mt-1" value={nom} onChange={(e) => setNom(e.target.value)} />
              </div>
              <div>
                <label className="label-industrial">Téléphone</label>
                <input className="input-underline w-full mt-1" value={telephone} onChange={(e) => setTelephone(e.target.value)} />
              </div>
              <div>
                <label className="label-industrial">Adresse</label>
                <input className="input-underline w-full mt-1" value={adresse} onChange={(e) => setAdresse(e.target.value)} />
              </div>
              <Button onClick={handleAdd} className="w-full bg-primary text-primary-foreground font-bold">Ajouter</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
          ...
          {clients.map((c) => (
            <motion.div
              ...
              className={`p-4 border-b border-border cursor-pointer transition-colors ${
                selectedClient === c.id ? "bg-accent/50" : ""
              }`}
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <p className="font-mono text-sm font-bold">{formatCFA(c.totalAchats)}</p>
                </div>
              </div>
            </motion.div>
          ))}
                ...
                clientVentes.map((v) => (
                  <div key={v.id} className="border border-border rounded p-3 space-y-2">
                    <div className="flex justify-between">
                      <span className="font-mono text-xs text-muted-foreground">{v.id} — {v.date}</span>
                      <span className="font-mono text-sm font-bold">{formatCFA(v.total)}</span>
                    </div>
                    {v.items.map((item, idx) => (
                      <div key={idx} className="flex justify-between text-sm text-muted-foreground">
                        <span>{item.nom} × {item.quantite}</span>
                        <span className="font-mono">{formatCFA(item.prixUnitaire * item.quantite)}</span>
                      </div>
                    ))}
                  </div>
                ))
